...
`setup/create_environment.sh` creates the fixed vanilla-LIBERO environment.
`setup/create_libero_plus_environment.sh` clones that Conda environment and
replaces only its LIBERO package with the pinned robustness fork. `data/build_libero_expert.sh`, `data/query_pi05_teacher.sh`, and
...
The LIBERO-Plus launcher instead uses `tmdpolicy-libero-plus`.
