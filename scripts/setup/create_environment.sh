...
conda run -n "${TMD_ENV_NAME}" python -m pip install \
...
conda run -n "${TMD_ENV_NAME}" python -m pip install -e "${TMD_PROJECT_ROOT}[test]"

MUJOCO_GL=egl conda run -n "${TMD_ENV_NAME}" \
